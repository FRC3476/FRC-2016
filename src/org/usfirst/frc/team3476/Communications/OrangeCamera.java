...
public class OrangeCamera {
   ...
   */
  public OrangeCamera(String cameraHost) {
  ...
  /**
   * Request a change in the exposure of the camera images.
   * @param exposure valid values 0 .. 100
   */
  public void writeExposure(int exposure) {
  ...
  /**
   * @return The configured exposure of the camera images
   */
  ...
  private Thread m_captureThread = new Thread(new Runnable() {
    ...
    public void run() {
      ...
      while (!m_done) {
        ...
        try {
          m_cameraSocket = OrangeCamera.this.createCameraSocket(requestString);
          OrangeCamera.this.readImagesFromCamera();
          consecutiveErrors = 0;
        } catch (IOException e) {
          consecutiveErrors++;
  ...
  private boolean writeParameters() {
    if (m_parametersDirty) {
      ...
      synchronized (m_parametersLock) {
        ...
        request += "&ImageSource.I0.Sensor.ExposureValue=" + m_exposure;
