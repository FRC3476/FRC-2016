...
public class GyroInterface
{
    ...
    public GyroInterface(SPI.Port port)
    {
        mSPI = new SPI(port);
    ...
    public void initializeGyro() throws GyroException
    {
        ...
        if (result != 1)
        {
            System.out.println(
                    "Unexpected self-check response: 0x" + Integer.toHexString(result) +
                            " errors: " + OrangeUtility.joinStrings(", ", extractErrors(result)));
        }
        ...
        {
            throw new GyroException("Gyro self-test didn't include all errors: 0x" + Integer.toHexString(selfCheckResult));
        }
    ...
    public static StatusFlag extractStatus(int result)
    {
        ...
        switch (stBits)
        {
            ...
            default:
                throw new RuntimeException("wtfbbq extractStatus");
        }
    }
